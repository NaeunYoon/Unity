...
public class PlatformSpawner : MonoBehaviour {
    ...
    void Start() {
        ...
        {
            //platformPrefab 을 원본으로 새 발판을 plloposition 위치에 복제 생성
    ...
    void Update() {
        ...
        {   //기록된 마지막 배치 시점을 현재 시점으로 갱신
            ...
            //다음 배치까지의 시간 간격을 timeBetSpawnMin, timeBetSpawnMax 사이에서 랜덤 결정
            timeBetSpawn = Random.Range(timeBetSpawnMin, timeBetSpawnMax);
            //배치할 위치의 높이를 yMin, yMax 사이에 랜덤 결정
            float yPos =Random.Range(yMin, yMax);

            //사용할 현재 순번의 발판 게임 오브젝트를 비활성화 하고 즉시 다시 활성화
            //이 때 발판의 paltform 컴포넌트의 onenable 메서드 실행
