...
public class _10_17_Inventory : MonoBehaviour
                            , IPointerDownHandler
                            , IDragHandler
                            , IPointerUpHandler

{
    public List<_10_17_Slot> slots;
    ...
    public void OnPointerDown(PointerEventData eventData)
    {
        ...
        {
            Debug.Log(slots[i].IsInRect(eventData.position));
            ...
            if (slots[i].IsInRect(eventData.position))
            {
                ...
                selectedSlotIndex = i;
                selectedItem.sprite = Resources.Load<Sprite>("Icons/" + slots[i].icon.sprite.name); //왜안대.. 왜 안따라와..
                //selectedItem.transform.position = eventData.position;


                selectedItem.gameObject.SetActive(true);    //이것두 안댐..
                selectedItem.transform.position = eventData.position;
    ...
    {
        if (selectedSlotIndex == -1)    //연산 줄이기 위한 조치(다른곳 클릭했을 때)
            return;
